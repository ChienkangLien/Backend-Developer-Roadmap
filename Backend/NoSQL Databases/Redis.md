...
4. 實時監控：當發現Redis 命中率驟降，排查訪問對象和訪問數據，可以設置黑名單限制服務
### 緩存擊穿
...
* 基於緩存的分布式鎖定：性能最高。利用緩存服務器來實現鎖定，例如使用Redis 的SETNX 或RedLock 演算法來實現，其中RedLock 是一種多重鎖定方式，能夠在不同節點之間避免競爭條件
* 基於ZooKeeper 的分布式鎖定：可靠性最高。使用ZooKeeper 叢集節點來實現，ZooKeeper 提供了順序節點以及watch 機制來實現分布式鎖定的獲取和釋放

```
127.0.0.1:6379> setnx k1 v1
-> Redirected to slot [12706] located at 192.168.191.139:6381
(integer) 1
192.168.191.139:6381> setnx k1 v2
(integer) 0 #寫入失敗
192.168.191.139:6381> del k1 #釋放鎖
(integer) 1
192.168.191.139:6381> setnx k1 v2
(integer) 1 #再次寫入，成功
192.168.191.139:6381> setnx k1 v3
(integer) 0
192.168.191.139:6381> expire k1 10 #設置過期時間
(integer) 1
192.168.191.139:6381> setnx k1 v3
(integer) 1 #10秒後釋放鎖
192.168.191.139:6379> set k2 v4 nx ex 10 #寫入key時同時設置過期時間，防止set和expire兩個操作時中間服務掛掉
OK
192.168.191.139:6379> setnx k2 v5
(integer) 1 #10秒後釋放鎖
```
Redis 能做到分布式鎖，主要也是基於單線程的特性，同一時刻只能有一個執行緒取得到同一個鎖。

為了確保分布式鎖可用，要滿足以下條件：
1. 互斥性。任何時刻，只有一個客戶端能持有鎖
2. 不會發生死鎖。即使有一個客戶端在持有鎖的期間崩潰而沒有主動解鎖，也要能保證後續其他客戶端能加鎖；設置過期時間
3. 加鎖和解鎖必須是同一個客戶端；UUID
4. 加鎖和解鎖必須是原子性操作；lua腳本
...
2. 使用`TTL`指令或`PTTL`指令，返回正整數表示鍵的剩餘過期時間，以秒為單位；-1表示鍵沒有設定過期時間；-2表示鍵不存在
...
#### 內存淘汰策略
當使用記憶體大於maxmemory 時，就會觸發Redis 主動淘汰記憶體方式
...
## Redis 6.0 新功能
### ACL
為使用者提供了更細粒度的安全控制機制。 ACL 允許你定義用戶，並為每個用戶分配特定的權限，以便控制哪些命令可以執行，哪些鍵可以存取等。
* ACL LIST：列出所有使用者及其權限
* ACL CAT \<category\>：列出所有<特定類別>可用的命令類別
* ACL WHOAMI：查看當前用戶
* ACL SETUSER：建立新用戶，並設定其權限

```
192.168.191.139:6380> acl setuser jack on >password ~boo:* +get #設置jack只能讀boo:開頭的key
OK
192.168.191.139:6380> auth jack password
OK
192.168.191.139:6380> set aa aa
(error) NOPERM this user has no permissions to run the 'set' command or its subcommand
192.168.191.139:6380> get aa
(error) NOPERM this user has no permissions to access one of the keys used as arguments
192.168.191.139:6380> get boo:aa
(nil)
```
### IO多線程
和Memcached 這種從IO處理到數據訪問多線程的實現模式有些差異。Redis 的多線程只是用來處理網路數據的讀寫和協議解析，執行命令仍然是單線程。

另外，多線程IO默認也是步開啟，需要在配置文件中配置
```
...
### 工具支持Cluster
舊版本想要搭建集群需要單獨安裝ruby 環境，自Redis 5 將redis-trib.rb 的功能集成到redis-cli。redis-benchmark 工具也支持cluster 模式，通過多線程的方式對多個分片進行測試。